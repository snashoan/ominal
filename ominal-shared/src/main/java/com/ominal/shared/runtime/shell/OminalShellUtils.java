package com.ominal.shared.runtime.shell;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import com.ominal.shared.errors.Error;
import com.ominal.shared.file.filesystem.FileTypes;
...
public class OminalShellUtils {

    private static final String LOG_TAG = "OminalShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                        ...
                        interpreter = OminalConstants.OMINAL_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link OminalConstants#OMINAL_TMP_PREFIX_DIR_PATH}. */
    public static void clearOminalTMPDIR(boolean onlyIfExists) {
        ...
        // ominal-reset (d6eb5e35). Moreover, TMPDIR must be a directory and not a symlink, this can
        // also allow users who don't want TMPDIR to be cleared automatically on ominal exit, since
        // it may remove files still being used by background processes (#1159).
        if(onlyIfExists && !FileUtils.directoryFileExists(OminalConstants.OMINAL_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            Logger.logInfo(LOG_TAG, "Not clearing ominal $TMPDIR");
        } else if (days == 0) {
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(OminalConstants.OMINAL_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to clear ominal $TMPDIR\n" + error);
            }
        } else {
            ...
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to delete files from ominal $TMPDIR older than " + days + " days\n" + error);
            }
        }
    }
