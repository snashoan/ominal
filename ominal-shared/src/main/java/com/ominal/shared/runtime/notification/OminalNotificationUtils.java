package com.ominal.shared.runtime.notification;
...
import com.ominal.shared.android.resource.ResourceUtils;
...
public class OminalNotificationUtils {
    /**
     * Try to get the next unique notification id that isn't already being used by the app.
     *
     * Ominal app and its plugin must use unique notification ids from the same pool due to usage of android:sharedUserId.
    ...
    public synchronized static int getNextNotificationId(final Context context) {
        if (context == null) return OminalPreferenceConstants.OMINAL_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;

        OminalAppSharedPreferences preferences = OminalAppSharedPreferences.build(context);
        if (preferences == null) return OminalPreferenceConstants.OMINAL_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;

        int lastNotificationId = preferences.getLastNotificationId();

        int nextNotificationId = lastNotificationId + 1;
        while(nextNotificationId == OminalConstants.OMINAL_APP_NOTIFICATION_ID || nextNotificationId == OminalConstants.OMINAL_RUN_COMMAND_NOTIFICATION_ID) {
            nextNotificationId++;
        }

        if (nextNotificationId == Integer.MAX_VALUE || nextNotificationId < 0)
            nextNotificationId = OminalPreferenceConstants.OMINAL_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;
    ...
    /**
     * Get {@link Notification.Builder} for ominal app or its plugin.
     *
     ...
     */
    @Nullable
    public static Notification.Builder getOminalOrPluginAppNotificationBuilder(final Context currentPackageContext,
        ...
        Notification.Builder builder =  NotificationUtils.geNotificationBuilder(ominalPackageContext,
            channelId, priority,
        ...
        // If a notification is to be shown by a ominal plugin app, then we can't use the drawable
        // resource id for the plugin app with setSmallIcon(@DrawableRes int icon) since notification
        // is shown with ominalPackageContext and ominal-app package would have a different id and
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            // Set drawable resource id used by ominal-app package
            Integer iconResId = ResourceUtils.getDrawableResourceId(ominalPackageContext, "ic_error_notification",
                ominalPackageContext.getPackageName(), true);
            if (iconResId != null)
