...
public class SystemEventReceiver extends BroadcastReceiver {
    ...
    public synchronized void onActionBootCompleted(@NonNull Context context, @NonNull Intent intent) {
        OminalShellManager.onActionBootCompleted(context, intent);
    }

    public synchronized void onActionPackageUpdated(@NonNull Context context, @NonNull Intent intent) {
        Uri data = intent.getData();
        if (data != null && OminalUtils.isUriDataForOminalPluginPackage(data)) {
            ...
            if (OminalFileUtils.isOminalFilesDirectoryAccessible(context, false, false) == null)
                OminalShellEnvironment.writeEnvironmentToFile(context);
        }
    }
