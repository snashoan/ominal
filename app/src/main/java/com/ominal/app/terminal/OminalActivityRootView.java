...
/**
 * The {@link OminalActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link OminalTerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `ominal.properties`
 * and {@link com.ominal.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for ominal, some keyboards may still show suggestions.
 ...
 * Also check https://github.com/ominal/ominal-app/issues/1539.
 *
 * This overlap may happen even without `enforce-char-based-input=true` for keyboards with extended layouts
 * like number row, etc.
 *
 * To fix these issues, `activity_ominal.xml` has the constant 1sp transparent
 * `activity_ominal_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link OminalActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class OminalActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public OminalActivity mActivity;
    ...
    private static final String LOG_TAG = "OminalActivityRootView";

    private static int mStatusBarHeight;

    public OminalActivityRootView(Context context) {
        super(context);
    }

    public OminalActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public OminalActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(OminalActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.getOminalActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addOminalActivityRootViewGlobalLayoutListener with a delay fails to
